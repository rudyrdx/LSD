...
public:
	...
	void reset() {
		...
		tick_count = 0;
		viewangles.Init();
		aimdirection =Vector(0.0f, 0.0f, 0.0f);
		sidemove = 0.0f;
		upmove = 0.0f;
		buttons = 0;
		impulse = 0;
...
public:
	...
	CUserCmd* get_usercmd(int slot, int sequence_number) {
		CInput* unk = this;

		if (slot != -1)
			unk = &this[0xDC * slot];

		auto ret = (CUserCmd*)(*((std::uintptr_t*) unk + 0x3D) + sizeof(CUserCmd) * (sequence_number % 150));
	...
	CUserCmd* get_usercmds(int sequence_number) {
		return (CUserCmd*)(*((std::uintptr_t*) this + 0x3D) + sizeof(CUserCmd) * (sequence_number % 150));
	}
};
