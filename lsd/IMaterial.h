...
public:
	...
	{
		ColorModulate(color.red / 255.f, color.green / 255.f, color.blue / 255.f);
		AlphaModulate(255.f);
	}
	void modulater(const Color color)
	{
		ColorModulate(color.red / 255.f, color.green / 255.f, color.blue / 255.f);
		AlphaModulate(0.5f);
	}
...
{
public:
	vfunc(84, find_material(const char* material, const char* group = nullptr), IMaterial*(__thiscall*)(IMaterialSystem*, const char*, const char*, bool, const char*))(material, group, true, nullptr)
...
#define find_mat(name) g_pMaterialSys->find_material(_(name))