...
struct C_Ray_Antario
{
	...
	C_Ray_Antario(Vector _start, Vector _end)
	{
		Init(_start, _end);
	}

	C_Ray_Antario(Vector _start, Vector _end, Vector _mins, Vector _maxs)
	{
...
public:
	...
	virtual void	ClipRayToEntity(const C_Ray_Antario &ray, unsigned int fMask, IHandleEntity *pEnt, C_Trace_Antario *pTrace) = 0;

	// Traces a ray against a particular entity
	virtual void	ClipRayToCollideable(const C_Ray_Antario &ray, unsigned int fMask, ICollideable *pCollide, C_Trace_Antario *pTrace) = 0;

	// A version that simply accepts a ray (can work as a traceline or tracehull)
	virtual void	TraceRay(const C_Ray_Antario &ray, unsigned int fMask, ITraceFilter *pTraceFilter, C_Trace_Antario *pTrace) = 0;
