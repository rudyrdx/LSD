...
{
	static auto oldorigin = g::pLocalEntity->GetOrigin();
	unpred_vel = (g::pLocalEntity->GetOrigin() - oldorigin) * (1.0 / g_pGlobalVars->intervalPerTick);
	oldorigin = g::pLocalEntity->GetOrigin();

	unpred_eyepos = g::pLocalEntity->GetEyePosition();
	...
	g_pPrediction->SetupMove(local_player, g::pCmd, g_pMoveHelper, &data);
	g_pMovement->ProcessMovement(local_player, &data);
	g_pPrediction->FinishMove(local_player, g::pCmd, &data);
	static auto pred_oldorigin = g::pLocalEntity->GetOrigin();
	pred_vel = (g::pLocalEntity->GetOrigin() - pred_oldorigin) * (1.0 / g_pGlobalVars->intervalPerTick);
	pred_oldorigin = g::pLocalEntity->GetOrigin();
...
{
	return g::pLocalEntity->GetTickBase();
}
